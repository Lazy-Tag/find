...
public class SplayTree {
    private static class Node { // 节点类
        ...
        public void print() {
            ...
            wifiList.stream().map(value -> "wifiName: " + value.getName() + ' ' + "Rssi: " + value.getRssi())
    ...
    private void splay(int x, int k) {
        while (node[x].parent != k) {
            int y = node[x].parent, z = node[y].parent;
            if (z != k) {
                if ((node[y].son[1] == x) ^ (node[z].son[1] == y))
                    rotate(x);
                else
                    rotate(y);
            }
    ...
    public void insert(String beacon, List<Wifi> list) {
        ...
        while (u != 0) {
            p = u;
            int k = 0;
            if (list.get(0).getName().compareTo(node[u].wifi) > 0)
                k = 1;
            u = node[u].son[k];
        }
        ...
        if (p != 0) {
            int k = 0;
            if (list.get(0).getName().compareTo(node[p].wifi) > 0)
                k = 1;
            node[p].son[k] = u;
        }
    ...
    private void getNodeList(int u, List<Node> list) {
        if (node[u].son[0] != 0)
            getNodeList(node[u].son[0], list);
        list.add(node[u]);
        if (node[u].son[1] != 0)
            getNodeList(node[u].son[1], list);
    }
